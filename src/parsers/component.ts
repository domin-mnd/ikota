...
/**
 * Create component file using the content & its name
 * @param {IkotaConfig} config Ikota configuration (may also be from flags)
 * @param {string} name Name of the component
 * @param {string} content Content of the file
 * @returns {Promise<void>} Creates a file with a component
 */
export async function createComponentFile(
  ...
  switch (config.preprocessor) {
    ...
    case "tailwind-css":
    case "none":
      path = `${config.componentPath || "."}/${name}/component.${
          ...
          if (pluginWithPreprocessor.components) {
            if (
              ...
              path = `${config.componentPath || "."}/${name}/${
                pluginWithPreprocessor.components[config.preprocessor].component
